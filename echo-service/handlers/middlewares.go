...
func WrapEndpoints(in svc.Endpoints) svc.Endpoints {
	...
	in.WrapAllLabeledExcept(middlewares.ErrorCounter(
		&generic.Counter{},
	))
	in.WrapAllLabeledExcept(middlewares.Latency(
		&generic.Histogram{},
	))
	in.WrapAllExcept(middlewares.LogMiddleware)
